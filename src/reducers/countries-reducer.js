import { GET_COUNTRIES, GET_COUNTRY, SEARCH_COUNTRIES, DELETE_COUNTRY, SET_CONTINENT, SET_COUNTRIES_PER_PAGE, SET_CURRENT_PAGE } from '../actions/actions-countries';
...
const countriesReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case DELETE_COUNTRY:
            const notDeletedCountries = state.countries.filter(country => country.id !==parseInt(action.id, 0));
            const notDeletedVisibleCountries = state.visibleCountries.filter(country => country.id !==parseInt(action.id, 0));
